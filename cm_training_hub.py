...
CONFIG_FILE = DATA_DIR / "hub_config.json"

# SharePoint team folder URL — CMs sync this via OneDrive
SHAREPOINT_URL = "https://amazon.sharepoint.com/sites/Model2Test/Shared%20Documents/Forms/AllItems.aspx?id=%2Fsites%2FModel2Test%2FShared%20Documents%2FCM%20Integration%20Reports%20%28test%29&viewid=5ac6c3d8%2D92af%2D4633%2Dad1a%2D40fedb290bd6"

# M1 access code — one-time password to unlock manager view
M1_ACCESS_CODE = "CM2026M1"
...
def load_hub_config():
    """Load hub configuration (alias, role, sync path, etc.)."""
...
def is_m1_role():
    """Check if current user has M1/manager role."""
    config = load_hub_config()
    return config.get("role") == "m1"


def get_sync_path():
    """Get the configured SharePoint sync path."""
    config = load_hub_config()
    path = config.get("sharepoint_sync_path", "")
...
# ============================================================
# SHAREPOINT SYNC — CM SIDE (individual progress file)
# ============================================================

def write_progress_excel(filepath, alias, history, oneone_topics_text, scenarios_viewed, checklist_state, stage):
    """Write a CM's living progress Excel file.
    
    Uses csv fallback if openpyxl is not available.
    """
    try:
        import openpyxl
        ...
        snapshot_data = [
            ...
            ["Total Check-ins", str(len(history))],
            ["Scenarios Practiced", f"{len(scenarios_viewed)}/6"],
        ]
        # Calculate overall confidence
        if history:
            latest = history[-1]
            avg = sum(latest["scores"].values()) / len(latest["scores"])
            snapshot_data[3][1] = f"{avg:.1f}/5 ({avg/5*100:.0f}%)"
        else:
            snapshot_data[3][1] = "No checks yet"

        for r, row in enumerate(snapshot_data, 1):
        ...
        dim_keys = ["trafficking", "optimizations", "communication", "prioritization",
                     "navigation", "ambiguity", "partnership", "independence"]
        headers2 = ["Date", "Stage", "Avg"] + [k.capitalize() for k in dim_keys] + ["Reflection"]
        for c, h in enumerate(headers2, 1):
            cell = ws2.cell(row=1, column=c, value=h)
            cell.font = header_font
            cell.fill = header_fill

        for r, entry in enumerate(history, 2):
            ws2.cell(row=r, column=1, value=entry.get("check_date", entry.get("date_display", "")))
            ws2.cell(row=r, column=2, value=entry.get("stage", ""))
            scores = entry.get("scores", {})
            avg = sum(scores.values()) / len(scores) if scores else 0
            ws2.cell(row=r, column=3, value=round(avg, 2))
            for i, key in enumerate(dim_keys):
                ws2.cell(row=r, column=4 + i, value=scores.get(key, ""))
            ws2.cell(row=r, column=4 + len(dim_keys), value=entry.get("reflection", ""))

        # -- Sheet 3: M1 Agenda Topics --
        ws3 = wb.create_sheet("M1 Agenda Topics")
        for c, h in enumerate(["Topic", "Raw Text"], 1):
            cell = ws3.cell(row=1, column=c, value=h)
            cell.font = header_font
            cell.fill = header_fill
        if oneone_topics_text and oneone_topics_text.strip():
            for r, line in enumerate(oneone_topics_text.strip().splitlines(), 2):
                if line.strip():
                    ws3.cell(row=r, column=1, value=line.strip())

        # -- Sheet 4: Milestones --
        ws4 = wb.create_sheet("Milestones")
        for c, h in enumerate(["Milestone", "Status", "Stage"], 1):
            cell = ws4.cell(row=1, column=c, value=h)
            cell.font = header_font
            cell.fill = header_fill
        row_num = 2
        for stage_key in ["s1", "s2", "s3"]:
            stage_label = {"s1": "Stage 1", "s2": "Stage 2", "s3": "Stage 3"}[stage_key]
            items = checklist_state.get(stage_key, {})
            for item_text, completed in items.items():
                ws4.cell(row=row_num, column=1, value=item_text)
                ws4.cell(row=row_num, column=2, value="✅ Complete" if completed else "⬜ Incomplete")
                ws4.cell(row=row_num, column=3, value=stage_label)
                row_num += 1
        ...
        # openpyxl not available — write a simple JSON fallback
        fallback = {
            "alias": alias,
            "stage": stage,
            "last_updated": datetime.now().isoformat(),
            "history": history,
            "oneone_topics": oneone_topics_text,
            "scenarios_viewed": list(scenarios_viewed),
...
def auto_sync_progress():
    """Sync current CM's progress to their SharePoint folder.
    
    Called silently after confidence checks, agenda edits, milestone toggles.
    Writes individual file only — M1 builds the aggregate on their side.
    """
    config = load_hub_config()
    if not config or config.get("uat_mode") or config.get("role") == "m1":
        return  # No sync in UAT mode

    sync_path = config.get("sharepoint_sync_path", "")
    alias = config.get("alias", st.session_state.get("alias", ""))

    if not sync_path or not os.path.exists(sync_path) or not alias:
        return
    ...
    try:
        ...
        # Determine stage
        stage = "Stage 1 — Building Trust"
        if history:
            stage = history[-1].get("stage", stage)

        # Write to CM's subfolder
        cm_folder = Path(sync_path) / alias
        cm_folder.mkdir(exist_ok=True)
        filepath = cm_folder / "cm_progress.xlsx"
        success = write_progress_excel(filepath, alias, history, oneone_topics_text,
                                        scenarios_viewed, checklist_state, stage)
        if success:
            st.session_state["last_synced"] = datetime.now()
            st.session_state["sync_warning"] = False
    except Exception:
        st.session_state["sync_warning"] = True


# ============================================================
# M1 ANALYTICS — Team Dashboard (reads all CM files)
# ============================================================

def scan_all_cm_progress(sync_path):
    """Scan all CM subfolders and read their progress files.
    
    Returns a list of dicts, one per CM.
    """
    cms = []
    sync = Path(sync_path)

    for cm_folder in sorted(sync.iterdir()):
        if not cm_folder.is_dir():
            continue
        # Skip known non-CM folders
        if cm_folder.name.startswith(".") or cm_folder.name.startswith("_"):
            continue

        progress_file = cm_folder / "cm_progress.xlsx"
        progress_json = cm_folder / "cm_progress.json"

        cm_data = {
            "alias": cm_folder.name,
            "stage": "Unknown",
        ...
        try:
            if progress_file.exists():
                cm_data = _read_cm_excel(progress_file, cm_data)
            elif progress_json.exists():
                cm_data = _read_cm_json(progress_json, cm_data)
            else:
                continue  # No progress file — skip this folder

            # Calculate "needs attention" flags
            cm_data = _calculate_attention_flags(cm_data)
            cms.append(cm_data)

        except Exception:
            # Broken file — still include CM but flag it
            cm_data["needs_attention"] = True
            cm_data["attention_reasons"].append("⚠️ Could not read progress file")
            cms.append(cm_data)

    return cms


def _read_cm_excel(filepath, cm_data):
    """Read a CM's progress Excel and populate cm_data dict."""
    try:
        import openpyxl
        ...
        if "Current Snapshot" in wb.sheetnames:
            ...
            for row in ws.iter_rows(min_row=2, max_col=2, values_only=True):
                if row[0] == "Current Stage":
                    cm_data["stage"] = row[1] or "Unknown"
                elif row[0] == "Overall Confidence":
                    val_str = str(row[1] or "")
                    if "/" in val_str:
                        try:
                            num = float(val_str.split("/")[0])
                            cm_data["overall_confidence"] = num
                            cm_data["confidence_pct"] = int(num / 5 * 100)
                        except ValueError:
                            pass
                elif row[0] == "Last Updated":
                    cm_data["last_updated"] = row[1]
                elif row[0] == "Total Check-ins":
                    try:
                        cm_data["total_checkins"] = int(row[1])
                    except (ValueError, TypeError):
                        pass
                elif row[0] == "Scenarios Practiced":
                    val_str = str(row[1] or "")
                    if "/" in val_str:
                        try:
                            cm_data["scenarios_done"] = int(val_str.split("/")[0])
                        except ValueError:
                            pass

        # Sheet 2: Confidence History
        if "Confidence History" in wb.sheetnames:
            ws = wb["Confidence History"]
            dim_keys = ["trafficking", "optimizations", "communication", "prioritization",
                         "navigation", "ambiguity", "partnership", "independence"]
            history_rows = []
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                for i, key in enumerate(dim_keys):
                    ...
                    if val is not None:
                        try:
                            entry["scores"][key] = int(val) if isinstance(val, (int, float)) else int(float(val))
                        except (ValueError, TypeError):
                            pass
                history_rows.append(entry)

            cm_data["confidence_history"] = history_rows
            if history_rows:
                cm_data["latest_scores"] = history_rows[-1].get("scores", {})
                if len(history_rows) >= 2:
                    cm_data["previous_scores"] = history_rows[-2].get("scores", {})

        # Sheet 3: M1 Agenda Topics
        if "M1 Agenda Topics" in wb.sheetnames:
            ws = wb["M1 Agenda Topics"]
            topics = []
            for row in ws.iter_rows(min_row=2, max_col=1, values_only=True):
                if row[0] and str(row[0]).strip():
                    topics.append(str(row[0]).strip())
            cm_data["agenda_topics"] = topics
            cm_data["open_agenda_items"] = len(topics)
        ...
        if "Milestones" in wb.sheetnames:
            ...
            for row in ws.iter_rows(min_row=2, max_col=3, values_only=True):
                ...
                total += 1
                status = str(row[1] or "")
                is_done = "Complete" in status or "✅" in status
                if is_done:
                    done += 1
                milestones.append({
                    "name": str(row[0]),
            ...
            cm_data["milestones"] = milestones
            cm_data["milestones_done"] = done
            cm_data["milestones_total"] = total
        ...
        cm_data["needs_attention"] = True
        cm_data["attention_reasons"].append("⚠️ Error reading Excel file")

    return cm_data


def _read_cm_json(filepath, cm_data):
    """Read a CM's JSON fallback progress file."""
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            data = json.load(f)
        cm_data["stage"] = data.get("stage", "Unknown")
        cm_data["last_updated"] = data.get("last_updated", "")
        history = data.get("history", [])
        cm_data["total_checkins"] = len(history)
        cm_data["scenarios_done"] = len(data.get("scenarios_viewed", []))
        ...
        if history:
            ...
            if scores:
                ...
                cm_data["overall_confidence"] = round(avg, 2)
                cm_data["confidence_pct"] = int(avg / 5 * 100)
                cm_data["latest_scores"] = scores
            if len(history) >= 2:
                cm_data["previous_scores"] = history[-2].get("scores", {})

            # Build simplified confidence history
            for entry in history:
                s = entry.get("scores", {})
                cm_data["confidence_history"].append({
                    "date": entry.get("check_date", entry.get("date_display", "")),
                    "stage": entry.get("stage", ""),
                    "avg": round(sum(s.values()) / len(s), 2) if s else 0,
                    "scores": s,
                })

        topics = data.get("oneone_topics", "")
        if topics and topics.strip():
            cm_data["agenda_topics"] = [l.strip() for l in topics.strip().splitlines() if l.strip()]
            cm_data["open_agenda_items"] = len(cm_data["agenda_topics"])

    except Exception:
        cm_data["needs_attention"] = True
        cm_data["attention_reasons"].append("⚠️ Error reading JSON file")

    return cm_data


def _calculate_attention_flags(cm_data):
    """Determine if a CM needs attention based on confidence drops and inactivity."""
    reasons = list(cm_data.get("attention_reasons", []))

    # Flag 1: Confidence DROP — any dimension dropped ≥1 point between last two checks
    latest = cm_data.get("latest_scores", {})
    previous = cm_data.get("previous_scores", {})
    if latest and previous:
        ...
        for key in latest:
            if key in previous:
                ...
                if diff <= -1:
                    dropped_dims.append(f"{key.capitalize()} {previous[key]}→{latest[key]}")
        if dropped_dims:
            reasons.append(f"📉 Confidence dropped: {', '.join(dropped_dims)}")

        # Overall confidence drop ≥ 10%
        latest_avg = sum(latest.values()) / len(latest) if latest else 0
        prev_avg = sum(previous.values()) / len(previous) if previous else 0
        if prev_avg > 0:
            pct_drop = (prev_avg - latest_avg) / prev_avg * 100
            if pct_drop >= 10:
                reasons.append(f"📉 Overall confidence dropped {pct_drop:.0f}%")

    # Flag 2: Inactivity — no update in 7+ days
    last_updated = cm_data.get("last_updated")
    if last_updated:
        try:
            if isinstance(last_updated, str):
                ...
                for fmt in ["%Y-%m-%d %H:%M", "%Y-%m-%d", "%Y-%m-%dT%H:%M:%S"]:
                    try:
                        last_dt = datetime.strptime(last_updated, fmt)
                        break
                    except ValueError:
                        continue
                else:
    ...
    # Flag 3: Very low confidence (any dim at 1)
    for key, val in latest.items():
        if val == 1:
            reasons.append(f"🔴 {key.capitalize()} at minimum (1/5)")

    cm_data["attention_reasons"] = reasons
    cm_data["needs_attention"] = len(reasons) > 0

    # Calculate trend emoji
    if latest and previous:
        latest_avg = sum(latest.values()) / len(latest)
        prev_avg = sum(previous.values()) / len(previous)
        if latest_avg > prev_avg + 0.2:
            cm_data["confidence_trend"] = "📈"
        elif latest_avg < prev_avg - 0.2:
            cm_data["confidence_trend"] = "📉"
        else:
            cm_data["confidence_trend"] = "📊"
...
def build_team_dashboard_excel(cms, filepath):
    """Build the M1 Team Dashboard Excel from all CM data."""
    try:
        import openpyxl
        from openpyxl.styles import Font, PatternFill, Alignment
        ...
        header_font = Font(bold=True, color="FFFFFF")
        header_fill = PatternFill(start_color="333333", end_color="333333", fill_type="solid")
        alert_fill = PatternFill(start_color="FFF3CD", end_color="FFF3CD", fill_type="solid")

        # -- Sheet 1: Team Overview --
        ws1 = wb.active
        ws1.title = "Team Overview"
        overview_headers = ["CM", "Stage", "Confidence", "Trend", "Last Active",
                            "Check-ins", "Scenarios", "Agenda Items", "Milestones", "Needs Attention"]
        for c, h in enumerate(overview_headers, 1):
            cell = ws1.cell(row=1, column=c, value=h)
            cell.font = header_font
            cell.fill = header_fill

        for r, cm in enumerate(sorted(cms, key=lambda x: x["alias"]), 2):
            stage_short = cm["stage"]
            if "Stage 1" in stage_short:
                stage_short = "Stage 1"
            elif "Stage 2" in stage_short:
                stage_short = "Stage 2"
            elif "Stage 3" in stage_short:
                stage_short = "Stage 3"

            ws1.cell(row=r, column=1, value=cm["alias"])
            ws1.cell(row=r, column=2, value=stage_short)
            ws1.cell(row=r, column=3, value=f"{cm['confidence_pct']}%" if cm["confidence_pct"] else "—")
            ws1.cell(row=r, column=4, value=cm["confidence_trend"])
            ws1.cell(row=r, column=5, value=cm.get("last_updated", "—"))
            ws1.cell(row=r, column=6, value=cm["total_checkins"])
            ws1.cell(row=r, column=7, value=f"{cm['scenarios_done']}/6")
            ws1.cell(row=r, column=8, value=cm["open_agenda_items"])
            ms = f"{cm['milestones_done']}/{cm['milestones_total']}" if cm["milestones_total"] else "—"
            ws1.cell(row=r, column=9, value=ms)
            attn = "; ".join(cm["attention_reasons"]) if cm["needs_attention"] else "—"
            ws1.cell(row=r, column=10, value=attn)
            if cm["needs_attention"]:
                for c in range(1, 11):
                    ws1.cell(row=r, column=c).fill = alert_fill

        for col_letter in ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"]:
            ws1.column_dimensions[col_letter].width = 18

        # -- Sheet 2: Competency Breakdown --
        ws2 = wb.create_sheet("Competency Breakdown")
        dim_keys = ["trafficking", "optimizations", "communication", "prioritization",
                     "navigation", "ambiguity", "partnership", "independence"]
        comp_headers = ["CM", "Stage"] + [k.capitalize() for k in dim_keys] + ["Avg"]
        ...
        for r, cm in enumerate(sorted(cms, key=lambda x: x["alias"]), 2):
            ws2.cell(row=r, column=1, value=cm["alias"])
            stage_short = "S1" if "Stage 1" in cm["stage"] else "S2" if "Stage 2" in cm["stage"] else "S3" if "Stage 3" in cm["stage"] else "?"
            ws2.cell(row=r, column=2, value=stage_short)
            scores = cm.get("latest_scores", {})
            for i, key in enumerate(dim_keys):
                ws2.cell(row=r, column=3 + i, value=scores.get(key, ""))
            if scores:
                ws2.cell(row=r, column=3 + len(dim_keys), value=round(sum(scores.values()) / len(scores), 2))
...
# ============================================================
# SETUP GATE — First-time setup page
# ============================================================

def render_setup():
    """Render the first-time setup gate."""
    st.markdown("")
    st.markdown("## 🎯 Welcome to the CM Integration Hub")
    st.markdown("")
    st.markdown(
        "This is your personal toolkit for ramping into campaign management. "
        "As you work through each stage, the hub tracks your progress and keeps "
        "your M1 in the loop — so they can spot trends, celebrate wins, and "
        "support where it counts."
    )
    st.markdown("")
    st.divider()
    st.markdown("### Let's get you connected")
    st.caption("This takes about 2 minutes. You'll only do this once.")
    st.markdown("")

    # Alias
    alias = st.text_input(
        "Your Amazon alias",
        placeholder="e.g., dcampos",
        key="setup_alias"
    )
    ...
    role = st.radio(
        "Your role",
        options=["Campaign Manager", "Manager / Training Lead"],
        index=0,
        key="setup_role"
    )
    ...
    # M1 password gate
    m1_code = ""
    if role == "Manager / Training Lead":
        m1_code = st.text_input(
            "🔐 Manager access code",
            type="password",
            help="This code is provided by your training program lead.",
            key="setup_m1_code"
        )
    st.markdown("")
    st.markdown(
        f"1. [Click here to open the team SharePoint library →]({SHAREPOINT_URL})\n"
        "2. In the toolbar, click the **⋯ (three dots)** menu, then select **Sync**\n"
        "   - If you see **Sync** directly in the toolbar, click that instead\n"
        "   - If you get a *'can't sync'* error, you're already synced — skip to step 3\n"
        "   - A pop-up may ask to open **Microsoft OneDrive** — click **Open**\n"
        "3. Your synced folder path should auto-fill below. If not, click **🔍 Find my synced folder**."
    )

    st.markdown("")

    # 🔍 Auto-detect OneDrive sync folder using alias template
    # SharePoint synced folders land under C:\Users\{alias}\OneDrive - amazon.com\{library name}
    detected_path = ""
    home = Path.home()
    onedrive_roots = sorted(home.glob("OneDrive*"), key=lambda p: p.name)

    # Auto-fill on first load — scan for Model 2 folders without requiring button click
    if "_detected_sync_path" not in st.session_state:
        candidates = []
        for od in onedrive_roots:
            try:
                for child in od.iterdir():
                    if child.is_dir() and "model" in child.name.lower():
                        candidates.append(child)
            except PermissionError:
                pass
        if candidates:
            st.session_state["_detected_sync_path"] = str(candidates[0])

    # Manual fallback button
    if st.button("🔍 Find my synced folder", key="setup_find_folder"):
        candidates = []
        for od in onedrive_roots:
            try:
                for child in od.iterdir():
        ...
        if candidates:
            st.session_state["_detected_sync_path"] = str(candidates[0])
            if len(candidates) > 1:
                st.info(f"Found {len(candidates)} possible folders. Using the first match — edit below if needed.")
            st.rerun()
        else:
            # Build the expected path as a hint
            expected = home / "OneDrive - amazon.com" / "Model 2 Test"
            st.error(
                f"Could not auto-detect. Expected something like:\n\n"
                f"`{expected}`\n\n"
                "Open **File Explorer** → navigate to your **OneDrive - amazon.com** folder → "
                "find the Model 2 folder → right-click → **Copy as path** → paste below."
            )

    # Pre-fill with detected path if available
    default_path = st.session_state.get("_detected_sync_path", "")
    sync_path = st.text_input(
        "Sync folder path",
        value=default_path,
        placeholder=f"e.g., C:\\Users\\{alias or 'youralias'}\\OneDrive - amazon.com\\Model 2 Test",
        key="setup_sync_path"
    )
    if default_path:
        st.caption("✅ Auto-detected — verify the path looks right, then click Complete Setup.")
    else:
        st.caption("⚠️ Paste the **local** folder path from File Explorer — not the SharePoint URL.")

    st.markdown("")

    # ── Complete Setup button ──
    if st.button("✅ Complete Setup", type="primary", key="setup_complete_btn"):
        errors = []
        if not alias or not alias.strip():
            errors.append("Please enter your Amazon alias.")
        if role == "Manager / Training Lead" and m1_code != M1_ACCESS_CODE:
            errors.append("Manager access code is incorrect. Check with your training program lead.")
        # Detect if user pasted a URL instead of a local path
        clean_path = sync_path.strip().strip('"') if sync_path else ""
        if clean_path and ("http://" in clean_path or "https://" in clean_path or "sharepoint.com" in clean_path):
            errors.append("That looks like a SharePoint URL — paste the **local folder path** from File Explorer instead. "
                          r"Example: `C:\Users\yourname\OneDrive - amazon.com\Model 2 Test - Documents`")
        elif clean_path and not os.path.isdir(clean_path):
            errors.append(f"Folder path not found: `{clean_path}`. Make sure OneDrive has finished syncing, "
                          "and that you're pasting the local folder path (right-click → Copy as path).")
        elif not clean_path:
            errors.append("Please paste your sync folder path, or click 🔍 Find my synced folder.")
        ...
        if errors:
            ...
            config = {
                ...
                "role": "m1" if role == "Manager / Training Lead" else "cm",
                "sharepoint_sync_path": clean_path,
                "setup_complete": True,
    ...
    st.divider()
    st.markdown("")
    if st.button("🧪 UAT Testing — Skip Setup", key="setup_uat_btn"):
        config = {
            "alias": alias.strip().lower() if alias else "uat_tester",
            "role": "cm",
        ...
        save_hub_config(config)
        st.session_state["alias"] = config["alias"]
        st.info("🧪 UAT mode active — SharePoint sync is disabled. All other features work normally.")
        st.rerun()

    st.caption("For testers only. SharePoint sync will not be active.")


# ============================================================
# M1 TEAM ANALYTICS — renders in Dashboard for M1 role
# ============================================================

def render_m1_analytics():
    """Render the M1 Team Analytics section on the Dashboard.
    Always shows the full skeleton — zero-state placeholders when no data exists."""
    ...
    if not sync_path:
        if config.get("uat_mode"):
            st.info("🧪 **UAT Mode** — Team analytics will populate once a SharePoint folder is connected.")
        else:
            st.warning("📂 Team folder not connected. Go to **⚙️ Settings → Reset Setup** to configure.")

    # -- Scan CM data (may be empty) ----------------------------------------
    cms = []
    if sync_path:
        cms = scan_all_cm_progress(sync_path)

    total_cms = len(cms)
    has_data = total_cms > 0

    # Helper for zero-state averages
    cms_with_data = [c for c in cms if c["total_checkins"] > 0] if has_data else []
    avg_confidence = (sum(c["confidence_pct"] for c in cms_with_data) / len(cms_with_data)) if cms_with_data else 0

    stage_counts = {"Stage 1": 0, "Stage 2": 0, "Stage 3": 0, "Unknown": 0}
    needs_attention_cms = []
    if has_data:
        for cm in cms:
            if "Stage 1" in cm["stage"]:
                stage_counts["Stage 1"] += 1
            elif "Stage 2" in cm["stage"]:
                stage_counts["Stage 2"] += 1
            elif "Stage 3" in cm["stage"]:
                stage_counts["Stage 3"] += 1
            else:
                stage_counts["Unknown"] += 1
        needs_attention_cms = [c for c in cms if c["needs_attention"]]
    ...
    with col1:
        st.metric("Total CMs", total_cms if has_data else "—")
    with col2:
        st.metric("Avg Confidence", f"{avg_confidence:.0f}%" if cms_with_data else "—")
    with col3:
        st.metric("Needs Attention", len(needs_attention_cms) if has_data else "—")
    with col4:
        st.metric("Completed Stage 3", stage_counts["Stage 3"] if has_data else "—")

    if has_data:
        st.caption(
            f"Stage 1: {stage_counts['Stage 1']}  •  "
            f"Stage 2: {stage_counts['Stage 2']}  •  "
            f"Stage 3: {stage_counts['Stage 3']}"
            + (f"  •  Unknown: {stage_counts['Unknown']}" if stage_counts["Unknown"] else "")
        )
    else:
        st.caption("Stage distribution will appear once CMs start submitting confidence checks.")
    ...
    if needs_attention_cms:
        for cm in sorted(needs_attention_cms, key=lambda x: len(x["attention_reasons"]), reverse=True):
            with st.expander(f"**{cm['alias']}** — {cm['stage']}", expanded=False):
                for reason in cm["attention_reasons"]:
                    st.markdown(f"  {reason}")
                if cm["latest_scores"]:
                    st.caption(f"Current confidence: {cm['confidence_pct']}%")
        st.info("💡 Consider prioritizing these CMs in your next 1:1s.")
    else:
        st.success("✅ No CMs flagged right now." if has_data else "No data yet — flags will appear when CMs show confidence drops or inactivity.")
    ...
    st.caption("Average confidence per dimension across all CMs, grouped by stage.")

    dim_keys = ["trafficking", "optimizations", "communication", "prioritization",
                 "navigation", "ambiguity", "partnership", "independence"]

    if has_data:
        for stage_label in ["Stage 1", "Stage 2", "Stage 3"]:
            stage_cms = [c for c in cms if stage_label in c["stage"] and c["latest_scores"]]
            if not stage_cms:
                continue

            cols = st.columns(len(dim_keys) + 1)
            with cols[0]:
                st.markdown(f"**{stage_label}**")

            for i, key in enumerate(dim_keys):
                with cols[i + 1]:
                    vals = [c["latest_scores"].get(key, 0) for c in stage_cms if c["latest_scores"].get(key)]
                    ...
                    if pct >= 75:
                        ...
                        emoji = "🔴"
                    st.caption(f"{emoji} {pct}%")

        # Legend
        st.caption("🔴 <50%  🟡 50-74%  🟢 75%+")

        # Find biggest gap
        all_dim_avgs = {}
        for key in dim_keys:
            vals = [c["latest_scores"].get(key, 0) for c in cms if c["latest_scores"].get(key)]
            if vals:
                all_dim_avgs[key] = sum(vals) / len(vals)
        if all_dim_avgs:
            weakest = min(all_dim_avgs, key=all_dim_avgs.get)
            st.info(f"💡 **Biggest team gap:** {weakest.capitalize()} ({all_dim_avgs[weakest]/5*100:.0f}% avg)")
    else:
        # Empty-state heatmap skeleton
        header_cols = st.columns(len(dim_keys) + 1)
        with header_cols[0]:
            st.markdown("**Stage**")
        for i, key in enumerate(dim_keys):
        ...
        for stage_label in ["Stage 1", "Stage 2", "Stage 3"]:
            row_cols = st.columns(len(dim_keys) + 1)
            with row_cols[0]:
                st.markdown(f"**{stage_label}**")
            for i in range(len(dim_keys)):
                with row_cols[i + 1]:
                    st.caption("⬜ —")
        st.caption("🔴 <50%  🟡 50-74%  🟢 75%+")
        st.info("💡 Heatmap will populate as CMs complete confidence checks.")
                ...
                try:
                    ...
                    if last_dt:
                        ...
                        if days <= 7:
                            active_7d += 1
                        if days >= 5:
                            inactive_cms.append((cm["alias"], days))
                except Exception:
                    pass

        ecol1, ecol2, ecol3 = st.columns(3)
        with ecol1:
            st.metric("Active this week", f"{active_7d}/{total_cms}")
        with ecol2:
            total_checkins = sum(c["total_checkins"] for c in cms)
            st.metric("Total check-ins (all CMs)", total_checkins)
        with ecol3:
            avg_checkins = total_checkins / total_cms if total_cms else 0
            st.metric("Avg check-ins per CM", f"{avg_checkins:.1f}")

        if inactive_cms:
            inactive_sorted = sorted(inactive_cms, key=lambda x: x[1], reverse=True)
            names = ", ".join([f"{name} ({days}d)" for name, days in inactive_sorted[:8]])
            extra = f" +{len(inactive_sorted) - 8} more" if len(inactive_sorted) > 8 else ""
            st.warning(f"💤 **Inactive 5+ days:** {names}{extra}")
    else:
        ecol1, ecol2, ecol3 = st.columns(3)
        with ecol1:
            st.metric("Active this week", "— / —")
        with ecol2:
            st.metric("Total check-ins (all CMs)", "—")
        with ecol3:
            st.metric("Avg check-ins per CM", "—")
        st.caption("Engagement data will appear once CMs begin using the hub.")

    st.divider()

    # ================================================================
    # 👥 ALL CMs — sortable, filterable, drill-down
    # ================================================================
    st.markdown(f"### 👥 All CMs ({total_cms})" if has_data else "### 👥 All CMs")

    # Sort / filter controls (always visible)
    sort_col, filter_col = st.columns(2)
    with sort_col:
        sort_by = st.selectbox("Sort by", ["Alias", "Confidence (low→high)", "Confidence (high→low)",
                                             "Last Active", "Stage"], key="m1_sort")
    with filter_col:
        filter_stage = st.selectbox("Filter by stage", ["All", "Stage 1", "Stage 2", "Stage 3"], key="m1_filter")

    if has_data:
        # Apply filter
        filtered = cms
        if filter_stage != "All":
            filtered = [c for c in cms if filter_stage in c["stage"]]

        # Apply sort
        if sort_by == "Confidence (low→high)":
            filtered = sorted(filtered, key=lambda x: x["confidence_pct"])
        elif sort_by == "Confidence (high→low)":
            filtered = sorted(filtered, key=lambda x: x["confidence_pct"], reverse=True)
        elif sort_by == "Last Active":
            filtered = sorted(filtered, key=lambda x: str(x.get("last_updated", "")), reverse=True)
        elif sort_by == "Stage":
            filtered = sorted(filtered, key=lambda x: x["stage"])
        else:
            filtered = sorted(filtered, key=lambda x: x["alias"])

        for cm in filtered:
            stage_short = "S1" if "Stage 1" in cm["stage"] else "S2" if "Stage 2" in cm["stage"] else "S3" if "Stage 3" in cm["stage"] else "?"
            attn_badge = " ⚠️" if cm["needs_attention"] else ""
            conf_display = f"{cm['confidence_pct']}%" if cm["confidence_pct"] else "—"

            with st.expander(
                f"{cm['confidence_trend']} **{cm['alias']}** — {stage_short} — {conf_display}{attn_badge}",
                expanded=False
            ):
                # Summary row
                ...
                with dc2:
                    st.metric("Check-ins", cm["total_checkins"])
                with dc3:
                    st.metric("Scenarios", f"{cm['scenarios_done']}/6")
                with dc4:
                    ms = f"{cm['milestones_done']}/{cm['milestones_total']}" if cm["milestones_total"] else "—"
                    st.metric("Milestones", ms)
                ...
                if cm["latest_scores"]:
                    st.markdown("**Confidence by Dimension:**")
                    score_cols = st.columns(4)
                    for i, (key, val) in enumerate(cm["latest_scores"].items()):
                        with score_cols[i % 4]:
                            bar = "🟩" * val + "⬜" * (5 - val)
                            prev_val = cm["previous_scores"].get(key)
                            delta = ""
                            if prev_val is not None and prev_val != val:
                                diff = val - prev_val
                                delta = f" ({'↑' if diff > 0 else '↓'}{abs(diff)})"
                            st.caption(f"{key.capitalize()}: {bar}{delta}")

                # Confidence history (mini trend)
                if len(cm["confidence_history"]) > 1:
                    st.markdown("**Trend:**")
                    trend_line = "  →  ".join(
                        [f"{h['date']}: {h['avg']:.1f}" for h in cm["confidence_history"][-5:]]
                    )
                    st.caption(trend_line)

                # Agenda topics
                if cm["agenda_topics"]:
                    st.markdown(f"**Open Agenda Items ({len(cm['agenda_topics'])}):**")
                    for topic in cm["agenda_topics"][:5]:
                        st.caption(f"📌 {topic}")
    else:
        ...
        st.dataframe(
            {
                "CM": ["—"], "Stage": ["—"], "Confidence": ["—"],
                "Last Active": ["—"], "Check-ins": ["—"], "Flags": ["—"]
            },
            use_container_width=True,
            hide_index=True
        )
        st.caption("CM rows will populate as team members complete their first confidence check.")
    ...
    if needs_attention_cms:
        for cm in needs_attention_cms[:5]:
            reasons_text = "; ".join(cm["attention_reasons"])
            st.markdown(f"📌 **{cm['alias']}** — {reasons_text}")
        ...
        if has_data:
            ...
            for key in dim_keys:
                vals = [c["latest_scores"].get(key, 0) for c in cms if c["latest_scores"].get(key)]
                if vals:
                    all_dim_avgs_coach[key] = sum(vals) / len(vals)
            if all_dim_avgs_coach:
                weakest = min(all_dim_avgs_coach, key=all_dim_avgs_coach.get)
                weak_pct = all_dim_avgs_coach[weakest] / 5 * 100
                if weak_pct < 60:
                    count = sum(1 for c in cms if c["latest_scores"].get(weakest, 5) <= 3)
                    st.info(
                        f"💡 **{count} CMs** are scoring ≤3 on **{weakest.capitalize()}** — "
                        f"consider a team-wide workshop or focused training session."
                    )
    else:
        st.caption("No specific recommendations yet. Coaching insights will appear when CMs have enough data to identify patterns." if not has_data else "✅ No CMs flagged — the team is on track.")
    ...
    with export_col1:
        if st.button("💾 Export Team Dashboard to Excel", key="m1_export", disabled=not has_data):
            export_path = sync_path / "M1_Team_Dashboard.xlsx"
            success = build_team_dashboard_excel(cms, export_path)
            if success:
                st.success(f"✅ Exported to: `{export_path}`")
            else:
                st.error("❌ Export failed. Make sure openpyxl is installed.")
    if not has_data:
        st.caption("Export will be available once CM data is present.")
...
def render_home():
    ...
    # ---- SYNC STATUS BANNER ----
    config = load_hub_config()
    if not IS_HOSTED and not config.get("uat_mode"):
        sync_path = get_sync_path()
        if sync_path:
            ...
            if last_synced:
                ...
                if mins_ago == 0:
                    st.caption("✅ Connected to team folder — synced just now")
                else:
                    st.caption(f"✅ Connected to team folder — last synced {mins_ago} min ago")
            else:
                st.caption("✅ Connected to team folder")
        elif config.get("sharepoint_sync_path"):
            st.caption("⚠️ Team folder not found — check your OneDrive sync")
    elif not IS_HOSTED and config.get("uat_mode"):
        st.caption("🧪 UAT Mode — SharePoint sync not active")

    # ---- M1 TEAM ANALYTICS (manager role only) ----
    if is_m1_role() and not IS_HOSTED:
        st.divider()
        render_m1_analytics()
        st.divider()
    ...
    if not alias:
        ...
        st.markdown(
            "1. 📊 **Check in on your confidence** — rate yourself across 8 key dimensions and track your growth over time\n"
            "2. 📋 **Look up your responsibilities** — see what you should Ask, Attempt, or Escalate at your current stage\n"
            "3. ⚡ **Prioritize competing tasks** — use 5 principles to decide what gets your attention first\n"
            "4. 💬 **Know who to contact** — follow the decision tree to the right person, channel, and message\n"
            "5. 🎯 **Practice real scenarios** — work through CM/CCM situations and compare your approach\n"
            "6. 📝 **Keep notes & prep for 1:1s** — capture learnings, build agendas, and download progress reports"
        )
        st.markdown("")
        st.info(
...
def render_campaign_navigation():
    ...
    if changed:
        ...
        # Auto-sync to SharePoint after milestone change
        auto_sync_progress()
        st.rerun()
...
def render_confidence_check():
    ...
    if st.button(t("cc_save_btn"), type="primary"):
        ...
        # Auto-sync to SharePoint after confidence check
        auto_sync_progress()
...
def render_notes():
    ...
    st.info("🔒 **Private** — these notes stay on your machine and are never shared with your manager or synced to the team folder.")
    st.divider()
    st.divider()
    ...
    if st.button("💾 Save 1:1 Topics", key="save_oneone_topics"):
        ...
        # Auto-sync to SharePoint after agenda edit
        auto_sync_progress()
...
def main():
    ...
    # ============================================================
    # SETUP GATE — check if first-time setup is complete
    # ============================================================
    if not IS_HOSTED and not is_setup_complete():
        render_setup()
        return  # Don't render anything else until setup is done

    # Load config and pre-populate alias from setup
    config = load_hub_config()
    if config.get("alias") and not st.session_state.get("alias"):
        st.session_state["alias"] = config["alias"]
